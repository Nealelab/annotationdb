...
def import_annotations_db(self, *args):
    ...
    response = urllib2.urlopen('https://storage.googleapis.com/annotationdb/master/annotationdb.json')
    meta = json.load(response)
    
    # separate variant and interval annotations
    variants = [x['id'] for x in meta if x['id'] in tops.keys() and x['type'] == 'variant']
    intervals = [x for x in meta if x['id'] in tops.keys() and x['type'] == 'interval']
